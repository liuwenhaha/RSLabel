...
class DetachableTabWidget(QtWidgets.QTabWidget):
    ...
    #  @pyqtSlot(QtWidgets.QWidget, QtCore.QString, QtGui.QIcon)
    ...
    class DetachedTab(QtWidgets.QDialog):
        onCloseSignal = pyqtSignal(
            QtWidgets.QWidget, QString, QtGui.QIcon)
    ...
    class TabBar(QtWidgets.QTabBar):
        ...
        def mouseDoubleClickEvent(self, event):
            event.accept()
            self.onDetachTabSignal.emit(self.tabAt(
                event.pos()), self.mouseCursor.pos())
        ...
        def mouseMoveEvent(self, event):
            ...
            if (((event.buttons() & QtCore.Qt.LeftButton)) and self.dragInitiated):
                ...
                # Stop the move event
                finishMoveEvent = QtGui.QMouseEvent(QtCore.QEvent.MouseMove, event.pos(
                ...
                if dropAction == QtCore.Qt.IgnoreAction:
                    event.accept()
                    self.onDetachTabSignal.emit(self.tabAt(
                        self.dragStartPos), self.mouseCursor.pos())

                # Else if the drag completed inside the tab bar, move the selected tab to the new position
                elif dropAction == QtCore.Qt.MoveAction:
                    if not self.dragDropedPos.isNull():
                        event.accept()
                        self.onMoveTabSignal.emit(self.tabAt(
                            self.dragStartPos), self.tabAt(self.dragDropedPos))
            else:
